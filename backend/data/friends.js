...
export const sendFriendRequest = async (senderName, recipientName) => {
    ...
    const senderFriends = senderData.friendList;

    //Check if users are already friends
    for (const friend of senderFriends) {
        if (friend.username === recipientName) {
            throw new RangeError("You are already friends with this user");
        }
    }
...
export const removeFriend = async (
    ...
    if (
        ...
        throw new RangeError(
            "You are not friends with this user. Cannot remove."
        );
    }
...
export const checkRequestStatus = async (senderUsername, recipientUsername) => {
    const userData = await getUserInfo(senderUsername, recipientUsername);
    const sender = userData.sender;
    const recipient = userData.recipient;

    const yourFriends = sender.friendList;
    const yourSent = sender.sentRequests;
